...
function process_test_dataset(input_dataset_file, options)
% PROCESS_ONE_SURFACE_TYPE_DATASET Entry point

arguments
    input_dataset_file {mustBeTextScalar, mustBeNonzeroLengthText} = ...
        "datasets/for_research_movement_direction/two_surface_types/green_table/exp02.csv",
    options.KalmanFilter logical = false
end

output_dataset_file = "output_files/datasets/for_research_movement_direction/two_surface_types/green_table/exp02.csv";
...
% column_units = replace_item(column_units, idx, "m");
mxpos = T.xpos * cameraParameters.x;
mypos = T.ypos * cameraParameters.y;
T = addvars(T, mxpos, mypos);
column_description = sprintf("%sX axis global position;Y axis global position", column_description);
column_units = sprintf("%sm;m", column_units);

% Calculate robot speed in local coordinate frame
[vx, vy, omega] = calc_local_robot_speed(T.t/1e3, T.mxpos, T.mypos, deg2rad(T.ang));
...
[filepath, ~, ~] = fileparts(output_dataset_file);
